...
set -euo pipefail
cd "$(dirname "$0")/.."
cats_executable="${1:-$PWD/build/Build/Products/Release/Cats.app/Contents/MacOS/Cats}"
cats_scratch="$(mktemp -d)"
printf 'Test logs: %s\n' "$cats_scratch/ui.log"
cp macos/Tests/Fixtures/state.json "$cats_scratch/cats-state.json"
# Isolate all telemetry and suppress collection. Never open real user sources.
CATS_DATA_DIR="$cats_scratch" CATS_EXTERNAL_COLLECTOR=1 "$cats_executable" > "$cats_scratch/ui.log" 2>&1 &
cats_test_pid=$!
